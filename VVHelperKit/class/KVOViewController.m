...
- (IBAction)addCount:(id)sender {
    ...
    self.complete (@"收到回馈信息(block)");
    
    if ([self.delegate respondsToSelector:@selector(sendMesage:)]) {
        [self.delegate sendMesage:@"收到回馈信息(代理)"];
    }
    
    if (self.delegateSignal) {
        [self.delegateSignal sendNext:@"收到回馈信息(RACSubject)"];
    }
}
